...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const db = client.db(config.mongodbDbName as string)
    ...
    // In caso di errore di connessione (es. variabile d'ambiente mancante o errore Atlas), 
    // restituiamo i dati demo invece di un errore 500 per permettere la visualizzazione del sito
    console.log('Restituisco dati demo a causa di un errore di connessione al database.')
    return {
      vehicles: getDemoVehicles(),
      warning: 'Database non connesso, visualizzazione dati demo.'
    }
  } finally {
    await client.close()
  }
