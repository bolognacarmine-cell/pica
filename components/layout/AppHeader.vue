...
const props = defineProps({
  logo: { type: String, default: '/logo-pica-caravan.jpg' },
  menuItems: { 
    type: Array, 
    default: () => [
      { name: 'Home', link: '/' },
      ...
      { name: 'Rimessaggio', link: '#rimessaggio' },
      { name: 'Azienda', link: '#azienda' },
      { name: 'Contatti', link: '#contatti' }
    ]
  },
  ctaText: { type: String, default: 'Richiedi Info' }
})
...
const initAnimations = () => {
  ...
  gsap.to('.nav-cta', {
    boxShadow: '0 0 25px rgba(241, 110, 34, 0.5)',
...
<template>
  ...
  >
    ...
    <div class="container mx-auto px-4 md:px-8 flex items-center justify-between h-full">
      ...
      <NuxtLink to="/" class="nav-logo flex items-center gap-4 group no-underline" aria-label="Pica Caravan - Home">
        <div class="nav-logo-parallax relative w-12 h-12 md:w-16 md:h-16 overflow-hidden group-hover:scale-105 transition-all duration-700">
          <nuxt-img 
            src="/logo-pica.png" 
        ...
        <div class="flex flex-col">
          <span class="text-xl md:text-2xl font-black tracking-tighter dark:text-white text-black leading-none uppercase">PICA CARAVAN</span>
          <span class="text-[9px] md:text-[10px] font-bold tracking-[0.4em] text-primary uppercase mt-1">Vendita & Rimessaggio</span>
        </div>
      </NuxtLink>
