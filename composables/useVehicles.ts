...
export const useVehicles = () => {
  ...
  const fetchVehicles = async () => {
    ...
    try {
      const { data } = await useFetch('/api/veicoli')

      if (data.value && Array.isArray(data.value.vehicles)) {
        vehicles.value = data.value.vehicles
      } else {
        vehicles.value = []
      }
    } catch (err) {
  ...
  const filteredVehicles = computed(() => {
    return vehicles.value.filter((v) => {
      ...
      const condition = (v.nuovaUsata || v.stato || '').toString().toLowerCase()

      const matchCondition =
        filters.value.condition === 'all' ||
        (filters.value.condition === 'nuove' && (condition.includes('nuov') || condition.includes('new'))) ||
        (filters.value.condition === 'usate' && (condition.includes('usat') || condition.includes('used')))
